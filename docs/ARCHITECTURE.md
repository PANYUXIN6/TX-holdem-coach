...
- M2.7 已在当前 Schema 上提供 Hand 与 Agent Foundation 审计持久化。`src/sessions/hand-audit/` 保存“开手命令前状态 + StartedHandFacts”和完整 M1.9 结算结果；`src/agents/audit/` 保存严格 Run Config、Budget、Attempt 载荷并定义固定 Player/Coach Decoder 端口。所有持久化 JSON 只保存行载荷版本；Execution Budget 因 M4.2 已规划 V2，例外保留显式版本注册表。两个 persistence Repository 只消费调用方事务，分别闭合 Hand 状态与 Foundation 审计事实。详细事实源见 [M2.7 历史设计](./superpowers/specs/2026-08-04-m2-7-hand-agent-audit-persistence-design.md)，当前事实以仓库地图和代码为准。
