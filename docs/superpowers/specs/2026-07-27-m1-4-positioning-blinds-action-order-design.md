...
> 2026-07-28 非 Agent 运行时重基线：本设计的按钮、庄盲、位置和顺时针算法全部继续有效。本模块继续接收显式标量 `completedHandCountBeforeStart`，不读取或依赖 `PrivateTableState`；M1.R 只调整纯状态类型。M1.9b 后续用 `poker-engine.ts.initializePokerTable()`/`startPokerHand()` 封装这些底层原语，M3 再从 `PrivateTableState.completedHandCount` 读取权威值并经门面传入。详见[非 Agent 运行时架构重基线](./2026-07-28-non-agent-runtime-architecture-rebaseline.md)。
