...
更新时间：2026-08-24（M0–M2、M3.1–M3.7、M4.1–M4.4 实施状态、首发前预建面收敛及远程测试分层已同步）
...
- M4.1–M4.2 已建立 Registry、预算、Coordinator/Worker、Run 队列、租约/fencing 与进程重启收敛；M4.3 已实现认证 Context/Prompt、静态 CapabilityExecutor、单一 DeepSeek ModelGateway 与 Attempt/Invocation 原子预算。M4.4 进一步实现单 Player Run authority 端口，在短事务内固定按 `Session → AgentRun` 共享锁读取当前快照、actor 和当前手事件，复验 Owner/决策身份/lease/fencing/deadline；Builder 与第一 Guard 从开手盲注种子逐事件复验 action before/after、金额证明和最终 Snapshot，再返回认证、深冻结且带规范哈希的 `PlayerVisibleState`。下注合法性、筹码转移、full raise/reopening 与行动后响应由 `poker/betting-projection.ts` 统一承载，权威引擎和观察链共同消费。第二、第三 Guard、业务 Runtime executor、Commit Gate 与 bootstrap 接线仍未实现。详细事实源见 [M4.3 设计](./superpowers/specs/2026-08-20-m4-3-context-capability-model-gateway-design.md)与 [M4.4 设计](./superpowers/specs/2026-08-23-m4-4-authoritative-player-observation-information-boundary-design.md)。
...
- `apps/server/test` 是非运行时测试层；`unit/` 与 `service/` 只验证离线领域、Codec、错误、HTTP/Provider 适配和应用端口，不连接真实数据库，coverage 也只收集这两层。远程 PostgreSQL 验收拆成两个受控入口：`db:test:*` 当前覆盖 M2.2–M2.8、M3.5 设置持久化及 M4.4 观察 Repository/共享锁；`postgres:e2e:*` 覆盖 M3.1–M3.7、M4.2–M4.4 的跨应用层流程，M4.4 从 leased/running Player Run 贯穿到认证观察。CLI 计划按 suite 拒绝未归属的里程碑，普通 `verify` 不收集任何远程入口。所有远程连接继续携带 Run ID 标签和数据库侧超时；阶段超时会沿 Vitest `AbortSignal` 终止迁移进程组和作用域内连接，并等待已登记清理完成。
...
共享协议只允许由两个应用依赖：`apps/web → packages/contracts ← apps/server`。私有人物模型配置、策略、数据库行与 Repository 类型不反向进入 Contracts。M4 当前依赖方向为 `persistence/player-observation-authority → sessions/authoritative-state 纯观察边界 → agents/player 认证观察端口 → 后续 Player Runtime`，观察边界与权威 `poker/betting.ts + hand-progression.ts` 共同向下依赖无 Runtime/SQL 知识的 `poker/betting-projection.ts`；另一条链为 `Player/Coach 私有 Runtime → Foundation Context/Capability/Gateway → model-gateway Provider Adapter`。Foundation、ModelGateway 与 Contracts 均不导入 Player 观察业务模块；`agents/player` 不导入完整私有状态或具体 PostgreSQL 适配器。Provider Adapter 不读取数据库；生产 Player Commit Gate 仍由 M4.7 实现。
...
## 已实现的 Agent 审计与 M4.1–M4.4 执行基础

M2.7–M4.2 已实现严格审计 Codec、静态 Registry、预算/Manifest、AgentRun 生命周期、Coordinator、数据库队列/租约/fencing 和双 lane Worker。M4.3 已实现通用 Context/Gateway 执行；M4.4 已实现权威 Player 观察与第一道信息防火墙。它仍不实现 Player 决策包、第二/第三 Guard、业务 Commit Gate，也不接入生产启动：

- 共享 Foundation 只机械认证 Runtime/Policy/Context/Prompt、执行代码预定 Capability 与同一 DeepSeek 的最多两次内容纠正；Player/Coach 的业务 Context section、Prompt 正文、输出 Schema、语义 Validator、Runtime executor 和 Gate 仍由后续里程碑实现。
- Player Runtime 负责“赢”。当前已实现的第一段链路为“捕获已领取 Run authority → 短事务共享锁定 Session/Run → current-only Codec → Hero 座位级白名单投影 → 第一 Guard 认证与观察哈希”；事务提交后才返回观察，模型与长计算不持锁。后续 M4.5/M4.6 才固化规则/spot/派生事实、候选、决策包与第二/第三 Guard，M4.7 再由专属 Commit Gate 输出扑克命令。
