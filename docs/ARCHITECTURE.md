...
更新时间：2026-08-25（M0–M2、M3.1–M3.7、M4.1–M4.5 完整实现、M4.5→M4.6 交接契约及远程测试分层已同步）
...
- M4.1–M4.2 已建立 Registry、预算、Coordinator/Worker、Run 队列、租约/fencing 与进程重启收敛；M4.3 已实现认证 Context/Prompt、静态 CapabilityExecutor、单一 DeepSeek ModelGateway 与 Attempt/Invocation 原子预算。M4.4 实现认证 `PlayerVisibleState` 与第一 Guard。M4.5 在同一共享下注内核上完成有限候选、贡献分层、Spot/手牌/可争夺池/指标/结果纯分析、静态 StrategyPack、低置信 heuristic、有界人物偏移、当前手 opponent evidence 与恒零 exploit v1；三个既有 Manifest grant 由固定 Capability Plan 顺序执行。最终聚合已保存三段权重、完整 heuristic 元数据和直接来源，严格 current decoder 在私有认证前复验 binding、候选守恒与哈希；Run 通过可逆审计依赖固化 StrategyPack，恢复只读 `pinnedRun`。M4.6 五项设计方案已确认但尚未开发，第二/第三 Guard、业务 Runtime executor、Commit Gate 与 bootstrap 接线仍未实现。详细事实源见 [M4.4 设计](./superpowers/specs/2026-08-23-m4-4-authoritative-player-observation-information-boundary-design.md)、[M4.5 设计](./superpowers/specs/2026-08-23-m4-5-player-deterministic-decision-preprocessing-design.md)与 [M4.6 设计](./superpowers/specs/2026-08-24-m4-6-player-decision-packet-bounded-choice-design.md)。
...
## Agent 审计与 M4.1–M4.5 核心执行基础

M2.7–M4.2 已实现严格审计 Codec、静态 Registry、预算/Manifest、AgentRun 生命周期、Coordinator、数据库队列/租约/fencing 和双 lane Worker。M4.3 已实现通用 Context/Gateway 执行；M4.4 已实现权威 Player 观察与第一道信息防火墙；M4.5 已完成不接生产 Worker 的确定性决策预处理及 M4.6 严格交接契约。当前仍不实现 Player 模型决策包、第二/第三 Guard、业务 Commit Gate，也不接入生产启动：
...
- Player Runtime 负责“赢”。当前已实现链路为“捕获已领取 Run authority → 短事务共享锁定 Session/Run → 第一 Guard 认证观察 → 独立短事务窄读目标 Hand 规则与 actor persona snapshot → 从 Run 恢复 pinned StrategyPack → 固定 Capability Plan → strict decoder/哈希认证预处理结果”；所有长计算在事务外，候选只来自服务端有限目录并保存 base/persona/exploit 三段权重和直接来源。策略无授权覆盖时明确走低置信 heuristic，人物每次转移最多 10%、单候选累计最多 20%，opponent evidence v1 只保留当前手计数且 exploit 恒为零。M4.6 再负责模型决策包与第二/第三 Guard，M4.7 由专属 Commit Gate 输出扑克命令。
