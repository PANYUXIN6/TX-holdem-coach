...
- `docs/superpowers/specs/2026-08-24-m4-6-player-decision-packet-bounded-choice-design.md`：M4.6 正式设计源；五项方案已于 2026-08-25 确认，包括专属三阶段 `player_decisions`、Memory 延后、仅 durable stage 恢复、M4.3 accepted-output 原子交接和封闭 Strategy v1 code。M4.6 尚未开始实现。
...
- `apps/server/test/`：`unit/` 与 `service/` 是离线层；前者覆盖领域、Codec、Repository/Coordinator 窄端口、M4.3 Context/Capability/Gateway、M4.4 观察/Guard 和 M4.5 决策预处理，后者覆盖 HTTP、Provider 与应用适配；`vitest.coverage.config.ts` 也只收集这两层。`integration/database-infrastructure.test.ts` 额外注册 `m44` 观察持久化与 `m45` Hand/persona 决策参考窄读，database m45 的依赖闭包禁止导入 Player 预处理。`integration/postgres-application-e2e.test.ts` 的 `m44` 从真实 Player Run 贯穿认证观察，`m45` 再贯穿 reference authority、固定三项 Capability Plan、三次能力审计与最终认证聚合，并验证预处理阶段不改 Session/Hand 事实或发布 SSE。两套入口只有显式启动器标记存在时才运行，默认 `verify` 均不收集。
...
- `apps/server/src/agents/player/` 与 `apps/server/src/agents/coach/`：各自拥有唯一当前 Definition、预算政策、Manifest grants 和独立认证 Route Policy。Player 已拥有观察与决策 reference 两个窄端口、观察绑定/source 映射、认证分析 core、Strategy/opponent 包装、heuristic/persona/exploit 政策、认证预处理结果，以及固定三项 Capability Definition/Plan；Plan 在 Executor clone 后从同一认证输入确定性重建并等值复验 fresh 私有实例。当前仍没有生产业务 Context/Prompt、第二/第三 Guard、Commit Gate 或 Runtime executor。
...
根 pnpm 脚本编排三个 workspace；`verify` 固定执行格式检查、类型检查与后端分类测试，且不读取模型 Key、数据库凭据或联网。Server 启动门仍止于 M3.7 的 HTTP/投影/SSE 组合。M4.4 已提供认证观察，M4.5 已提供不接 Worker 的确定性预处理与 Capability Plan；M4.6 决策包/后两道 Guard/唯一生产 Runtime executor、M4.7 Commit Gate 与 `bootstrap.ts` 接线仍不存在，因此默认启动和 `verify` 均不会发起模型请求。
