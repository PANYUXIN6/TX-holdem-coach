...
- 状态：进行中；M0–M2、M3.1–M3.7、M4.1–M4.10、M5.1–M5.5、M6.1–M6.6 和 M7.1–M7.6 已实现。M3.8 主体接线已由 M4.10 落地，专项收口仍待完成；前端真机及明确列出的人工验收仍待完成。M7.6 必要远程定向验收已通过（m55 PostgreSQL E2E 于 2026-09-14 由用户确认）；M7.7–M9 待开发，M10/M11 为分阶段后置能力。各任务的历史测试范围见对应实施记录，不以 milestone 代替 full。
- 日期：2026-07-23
- 最后更新：2026-09-14
- 本文不包含工期、人数或里程碑时间估算。
...
设计文档：[M7.7 历史页与本手流程设计](../specs/2026-09-14-m7-7-history-current-hand-detail-design.md)（设计稿待审阅；包含当前动作金额投影增量、URL 筛选与分页、显式揭牌、本手终局/导航及 A–E 研发编排，尚未实施）。
