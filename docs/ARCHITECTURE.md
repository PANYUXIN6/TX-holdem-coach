...
- M4.1–M4.2 已建立 Registry、预算、Coordinator/Worker、Run 队列、租约/fencing 与进程重启收敛。M4.3 在 Foundation 内恢复有真实执行消费者的认证 Context/Prompt、静态 CapabilityExecutor 和单一 DeepSeek ModelGateway，并在独立 Provider 适配层锁定无工具、无思考、无 SDK 重试的结构化调用；持久化层以父 Run 锁原子裁决 Attempt 与 Invocation 预算，先提交 Attempt/Invocation 预留再执行外部调用，并在 Attempt finish 时用实际 Token/成本替换预留、超额则保留审计但拒绝输出。Worker 可构造但仍没有业务 Runtime executor、Commit Gate 或 bootstrap 接线。详细事实源见 [M4.3 设计](./superpowers/specs/2026-08-20-m4-3-context-capability-model-gateway-design.md)。
