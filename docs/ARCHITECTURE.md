...
更新时间：2026-08-17（M0–M2、M3.1–M3.7、M4.1–M4.2 实施状态及 Coach 后置规划已同步）
...
- M2.7 已在当前 Schema 上提供 Hand 与 Agent Foundation 审计持久化。`src/sessions/hand-audit/` 保存“开手命令前状态 + StartedHandFacts”和完整 M1.9 结算结果；`src/agents/audit/` 保存严格 Run Config、完整 Execution Budget、Attempt 载荷并定义固定 Player/Coach Decoder 端口。所有持久化 JSON 只保存首发 current 行载荷版本，不保留 legacy 注册、迁移或 V1/V2 兼容分支。两个 persistence Repository 只消费调用方事务，分别闭合 Hand 状态与 Foundation 审计事实。详细事实源见 [M2.7 历史设计](./superpowers/specs/2026-08-04-m2-7-hand-agent-audit-persistence-design.md)，当前事实以仓库地图和代码为准。
- M4.1 已新增 `src/agents/foundation/`、Player/Coach 唯一当前定义及 `production-runtime-registry.ts` 组合点；M4.2 在同一 Foundation 边界新增事务绑定 Coordinator、双 lane Worker 与窄控制端口，并在 `src/persistence/agent-run-lifecycle-repository.ts` 落地 current-only Run 持久化、PostgreSQL 原子领取、容量裁决、租约/fencing、接管和通用终态。既有 Attempt/Capability writer 现复验不可伪造 authority。Worker 可构造但没有 ModelGateway、业务 Commit Gate 行为或 bootstrap 接线。详细事实源见 [M4.1 设计](./superpowers/specs/2026-08-14-m4-1-agent-foundation-core-protocol-static-registry-design.md)与 [M4.2 设计](./superpowers/specs/2026-08-16-m4-2-agent-run-persistence-coordinator-worker-design.md)。
...
- `apps/server/test` 是非运行时测试层；Vitest 以 Node 环境和 V8 coverage 运行 `unit/`、`integration/` 与 `service/` 分类。普通 `verify` 明确不收集远程数据库测试文件。受控启动器支持迁移-only、单个 `m22…m28`/`m31`…`m37`/`m42`、显式遗留事务清理和最终 full，并在首个失败或阶段超时后停止调度后续里程碑；`m42` 以真实 PostgreSQL 验收 AgentRun 并发创建、Player 设置锁、活动唯一、运行时隔离容量、租约/恢复/fencing、终态竞争、旧 Attempt 收敛和损坏行跨批扫描。所有测试连接继续携带 Run ID 标签和数据库侧超时。
...
## 已实现的 Agent 审计与 M4.1–M4.2 运行基础

M2.7 已实现严格审计 Codec、Foundation Repository 与 Runtime Decoder 组合端口；M4.1 已实现执行状态机、静态 Registry、预算/能力/Context 协议和 Player/Coach 隔离端口；M4.2 已实现 current-only AgentRun 生命周期 writer、事务绑定 Coordinator、数据库队列/租约/fencing、恢复资格与显式双 lane Worker。它仍不调用模型、不实现业务 Commit Gate，也不接入生产启动。后续实现以 [Agent Foundation 与受限 Runtime](./superpowers/specs/2026-07-26-agent-foundation-runtime-architecture.md)、[M4.1 设计](./superpowers/specs/2026-08-14-m4-1-agent-foundation-core-protocol-static-registry-design.md)、[M4.2 设计](./superpowers/specs/2026-08-16-m4-2-agent-run-persistence-coordinator-worker-design.md)、[Agent 大模块开发任务](./superpowers/plans/2026-07-26-agent-module-development-tasks.md) 和 Player/Coach 专项设计为准：

- 共享 Foundation 提供静态 Runtime 注册、不可变预算、默认拒绝能力授权、Context 信封、执行状态、AgentRun Coordinator/Worker 和 authority 协议，不理解扑克目标；模型网关、Player/Coach Runtime 执行与业务 Gate 仍由后续里程碑实现。
