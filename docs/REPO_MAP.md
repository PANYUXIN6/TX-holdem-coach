...
- `apps/server/scripts/database-test-plan.mjs`：远程 PostgreSQL 测试 CLI 的纯计划边界；`database` suite 只接受迁移、cleanup、full 或已登记里程碑（含 `m410`），`e2e` suite 同样只接受已登记里程碑（含 `m410`），并拒绝未归属的名称。计划分别选择 `database-infrastructure.test.ts` 或 `postgres-application-e2e.test.ts`，再生成唯一 Run ID、显式 scope 与失败即停的 Vitest 参数。
...
- `apps/server/eval/player/`：离线确定性 Player Eval 的唯一入口；严格固定场景先由真实扑克引擎构造权威输入，再实际执行观察、认证、分析、候选、Guard、Memory、Context 与 Prompt 纯链，并以独立确定性/安全 grader 断言结果。fingerprint 同时覆盖场景、grader、runner 与被执行的生产源，默认 `verify` 仅运行该离线 Eval。
