...
- 验证少于 5 个或多于 8 个 AI、重复人物、重复座位、座位超出 `0..8` 以及少于 6 席或多于 9 席的公开快照被拒绝。
- 不为每个字符串字段重复同构用例。
...
- 支持 6–9 人各发两张底牌。
- 翻牌、转牌和河牌前各 burn 一张。
- 返回可审计的完整牌堆顺序、底牌、公共牌和 burn card。
...
- Agent 运行事件可以在同一 `stateVersion` 下继续增加 `eventSeq`。
- 纯 Agent 运行事件只更新 `sessions` 协调字段和统一事件，不重写未变化的私有扑克快照。
- 事务提交后才能向发布层返回可发送事件。

后端测试闭环：

- 在每个写入阶段注入失败，断言全部回滚。
- 覆盖同一扑克版本的多条 Agent 运行事件。
...
- 保存完整牌堆、burn card、全部底牌、公共牌、起止筹码和结算。
...
- 校验 5–8 个不同 `personaId`、唯一座位、手动或随机座位和必需的 `OwnerScope`；当前身份适配器固定为 `local-user`。
- DeepSeek Key 缺失时阻止创建；Kimi Key 缺失时返回警告。
- 从当前预设人物目录或上一场配置快照创建全新的 `session_agents` 和空记忆；沿用上一场时不升级人物版本。
- 每个 `OwnerScope` 同一时间只允许一个活动场次；不同 Owner 的独立单人牌桌不互相阻塞。
- 所有座位以 2,000 筹码开始。

后端测试闭环：

- 覆盖 6–9 人合法组桌，以及人数越界、未知人物、重复人物、重复座位和座位号越界。
...
- 直接结束场次不触发 AI 自动买入。
- 任意正余额可以开始下一手。
- 结束场次只把 `sessions.lifecycleStatus` 改为 `ended` 并递增 `eventSeq`，不改写最终私有扑克快照或递增 `stateVersion`；随后销毁可运行 Agent。
...
- 人物 API 只提供列表和详情读取，不提供创建、修改、复制或删除端点；目录响应不暴露 Player Runtime 私有人物提示或不必要的完整模型参数。
...
- 八类 SSE 事件统一使用 `payload: { snapshot }`；`type` 仅表示已持久化事件原因，不引入按类型分支的 SSE 负载。
...
- 断言全部八类事件都使用相同的公开快照负载，并能仅靠快照驱动行动序列、结果摘要和运行状态展示。
...
- 启动时读取活动场次和最新快照。
- `thinking` 的持久化 AgentRun 保持未完成；旧租约到期后允许新 Worker 以新 fencing token 领取。
- 领取后重新读取权威状态；仍匹配时按固定版本重建决策包，已变化时旧运行标记 stale，并仅在当前仍需 AI 行动时创建替代运行。
- `paused` 保持暂停，等待人工重试。
...
- 实现持久化后执行、进程内 Worker、租约、fencing、取消、恢复和并发限制。
...
- 复验候选、金额、来源版本、OwnerScope、租约、fencing、actorSeat 和当前权威状态。
...
- 读取后端只读人物目录，展示彩色文字头像、名称、风格摘要和模型配置摘要。
...
- 暂停时冻结操作并提供错误摘要、查看调试信息、重新请求当前 AI 行动和结束场次。
- 调试视图展示脱敏调用链，不显示隐藏推理。
...
- 支持 `public` 与已结束手牌的显式 `auditReveal`。
- 可以打开关联 Agent 调用链。
...
- `HandReviewCaseBuilder` 只从已结束内部手牌和权威历史事实构建复盘案例。
...
- `POST /api/hands/:id/coach-reviews` 只接受已结束手牌及幂等 `requestId`；提供手牌复盘列表与单份报告查询。
...
- 覆盖浏览器式重连、重复命令、服务进程重建、`thinking` 自动新请求、`paused` 保持暂停和损坏快照只读诊断。
...
- 对一手有覆盖基准和一手无覆盖基准的已结束手牌请求 Coach，检查逐决策分析、信息边界、频率/尺度文案、失败重试和范围矩阵。
