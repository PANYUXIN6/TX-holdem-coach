...
- `apps/web/vitest.config.ts`、`apps/web/test/navigation.test.ts`：独立 Node 导航冒烟，只收集 Web `test/**/*.test.ts`；根 `test:web` 独立执行，`test` / `verify` 在后端测试之后执行。M6.2 已装配 Query，M6.4 已接入页面作用域 UI Store；M7.1–M7.9 已接入实体读取和产品控件；统计与设置路由均接入真实查询。
...
- `apps/server/scripts/database-test-plan.mjs`：远程 PostgreSQL 测试 CLI 的纯计划边界；`database` suite 接受独立的 `--connections` 故障回归入口，以及迁移、cleanup、full 或已登记里程碑（含 `m410`、`m55` 与仅 database 的 `m51`），`e2e` suite 只接受已登记的跨层里程碑（含 `m55`），并拒绝未归属的名称。计划分别选择 `database-infrastructure.test.ts` 或 `postgres-application-e2e.test.ts`，再生成唯一 Run ID、显式 scope 与失败即停的 Vitest 参数。
...
- `apps/server/test/integration/database-test-harness.ts`、`database-test-runtime.ts`、`database-test-client.ts`、`src/db/database-test-suite-lock.ts` 与 `README.md`：分别拥有远程阶段注册/迁移准备、取消与夹具恢复编排、事务内配置校验/取消后 SQL 拒绝、套件锁，以及操作规则。套件锁独占 migration `5432` 连接并以 session advisory lock、backend PID 心跳和连接关闭信号保持 fail-closed；业务事务仍走 runtime `6543` pooler，每次 `begin` 使用 `SET LOCAL`/`SHOW` 认证标签和超时，并保留随后设置隔离级别的能力。取消时先有界停止工作连接，再通过独立受保护连接恢复已登记夹具，最后关闭清理连接；锁丢失后禁止清理写入。预检报告有标签的冲突连接及未标记的应用持锁事务，自动终止仅适用于有测试标签的连接。`database-connection-assertions.ts` 经独立 `database-connections.test.ts` 或 database m22/full 验证真实配置、空闲事务锁释放、取消回滚和独立连接清理，不修改应用数据。共享能力继续提供事务 PID、JSONB fixture、主失败保留及脱敏诊断。
...
- `apps/web/src/history/HistoryPage.tsx` / `HistoryFilters.tsx`：历史卡片按当前页相邻场次分组；原 `historySearch` 与 `queries.hands` 拥有已应用筛选和游标，抽屉只保存草稿。场次/人物选项按需分页读取原 `queries.sessions`，固化配置按 key 去重。
- `apps/web/src/history/HandPages.tsx`：完成手默认 public，仅本页主动操作挂载 audit observer；URL、手牌或页面切换撤销揭牌意图。当前手直接订阅唯一 Session Query，租用仍归 SessionRouteBridge；终局只由匹配的完成摘要切换到 public 完成手查询。
...
- `apps/web/src/settings/SettingsPage.tsx` / `SettingsTime.tsx` / `settings.css`：`Pages` 的设置入口，独立 Provider/Health 读取、手动检测、调用审计与原清空 Host；仅展示公开摘要和本地时间。
- `apps/web/src/settings/BudgetForm.tsx` / `adapter.ts`：字符串预算草稿、完整 Schema 校验、差异 PATCH、后台读取与脏草稿协调，保持原 Mutation 刷新回调。
