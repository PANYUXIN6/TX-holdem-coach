...
const PrivateEventV2AdditionSchema = z.discriminatedUnion('type', [
...
export function createPrivateEventV2(input: unknown): PrivateEventV2 {
  try {
    if (
      typeof input === 'object' &&
      input !== null &&
      ...
      return createPrivateEventV1(input)
    }
    return deepFreeze(
      structuredClone(PrivateEventV2AdditionSchema.parse(input)),
    )
  } catch {
...
export function getPrivateEventHandId(event: PrivateEventV2): string | null {
  switch (event.type) {
    case 'handStarted':
