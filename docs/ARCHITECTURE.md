...
- M4.1–M4.2 已建立 Registry、预算、Coordinator/Worker、Run 队列、租约/fencing 与进程重启收敛。M4.3 在 Foundation 内恢复有真实执行消费者的认证 Context/Prompt、静态 CapabilityExecutor 和单一 DeepSeek ModelGateway，并在独立 Provider 适配层锁定无工具、无思考、无 SDK 重试的结构化调用；Adapter 与最终语义 Validator 的输出都必须通过敏感扫描，命中时只以稳定替代投影和允许的 usage/finish reason 完成拒绝审计。持久化层以父 Run 锁原子裁决 Attempt 与 Invocation 预算，先提交 Attempt/Invocation 预留再执行外部调用，并在 Attempt finish 时用实际 Token/成本替换预留、超额则保留审计但拒绝输出。Worker 可构造但仍没有业务 Runtime executor、Commit Gate 或 bootstrap 接线。详细事实源见 [M4.3 设计](./superpowers/specs/2026-08-20-m4-3-context-capability-model-gateway-design.md)。
...
共享协议只允许由两个应用依赖：`apps/web → packages/contracts ← apps/server`。私有人物模型配置、策略、数据库行与 Repository 类型不反向进入 Contracts。M4 当前依赖方向为 `Player/Coach 后续私有 Runtime → Foundation Context/Capability/Gateway 协议 → model-gateway Provider Adapter`，以及 `Foundation 控制端口 ← persistence 事务绑定 → postgres.js`。M4.3 新增的 Context/Capability/Gateway 协议不导入 SQL、Drizzle Schema、HTTP、Provider SDK 或扑克私有状态；M4.2 既有 Coordinator 仍通过窄 Repository/事务绑定编排 Run。Provider Adapter 不读取数据库；生产 Player/Coach Commit Gate 仍由 M4.7/M8 实现。
...
## 已实现的 Agent 审计与 M4.1–M4.3 通用执行基础

M2.7–M4.2 已实现严格审计 Codec、静态 Registry、预算/Manifest、AgentRun 生命周期、Coordinator、数据库队列/租约/fencing 和双 lane Worker。M4.3 已实现确定性 Context/Prompt 准备、静态 CapabilityExecutor、Player/Coach 独立 Route Policy、DeepSeek Adapter、单 Provider 有界纠错、Token/成本记账与 Attempt 原子预算控制。它仍不实现业务 Commit Gate，也不接入生产启动：
