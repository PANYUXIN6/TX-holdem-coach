...
- M4.1–M4.5 已建立通用 Agent Foundation、权威 Player 观察与确定性预处理。M4.6 进一步实现完整 `DecisionAuditSnapshot`、32 项事实清单、最小 `PlayerModelProjection`、第二/第三 Guard、静态 Prompt、bounded-choice Validator、三阶段 `player_decisions`、accepted Attempt/selected 原子交接及唯一 Player executor。M4.7 通过 `createPlayerCommitSessionComposition()` 将认证结果经纯 Validator 映射为仅服务端可达的 `aiAction`，并在固定 `Session → Hand → AgentRun → PlayerDecision → Attempt` 锁序中以持久化层返回的无业务语义锁定事实复验后，与牌局 mutation、ledger、Decision committed、Run completed 原子提交；Gate 工厂、私有 executor 与 Player/Repository 装配全为 Session 命令模块局部实现，组合物只暴露公开 commands 与 `Gate.commit(authority, result)`，Persistence 不反向依赖 Player Validator。其验收根因修复与远程证据尚在进行，未接入 `bootstrap.ts`，M4.8/M4.10 仍为上线门禁。详细事实源见 [M4.7 设计](./superpowers/specs/2026-08-25-m4-7-player-validator-command-commit-gate-design.md)。
