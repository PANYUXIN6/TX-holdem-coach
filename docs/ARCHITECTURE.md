...
更新时间：2026-08-18（M0–M2、M3.1–M3.7、M4.1–M4.2 实施状态及首发前预建面收敛已同步）
...
- M4.1 已新增 `src/agents/foundation/`、Player/Coach 唯一当前定义及 `production-runtime-registry.ts` 组合点；M4.2 在同一 Foundation 边界新增事务绑定 Coordinator、双 lane Worker 与窄控制端口，并在 `src/persistence/agent-run-lifecycle-repository.ts` 落地 current-only Run 持久化、PostgreSQL 原子领取、容量裁决、租约/fencing 与进程重启收敛（`process_restart`）。跨 owner 不做 checkpoint 接管；`agent_run_recovery_rejected` 会收敛为 `cancelled(process_restart)`。既有 Attempt/Capability writer 现复验不可伪造 authority。没有真实 Runtime Codec 的 checkpoint/result、无消费者聚合 Reader，以及未参与执行/恢复的 ContextEnvelope、Capability executor 和状态机均已删除。Worker 可构造但没有 ModelGateway、业务 Commit Gate 行为或 bootstrap 接线。详细事实源见 [M4.1 设计](./superpowers/specs/2026-08-14-m4-1-agent-foundation-core-protocol-static-registry-design.md)与 [M4.2 设计](./superpowers/specs/2026-08-16-m4-2-agent-run-persistence-coordinator-worker-design.md)。
