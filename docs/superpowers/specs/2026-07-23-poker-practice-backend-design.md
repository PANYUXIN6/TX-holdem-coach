...
当前规则集规范值为 `nlhe-cash-6to9-10-20-v1`。每手开始时由开手编排把该值固化进当前 Hand 检查点；后续 Player、Coach、策略查询和检查点恢复只读取目标手牌绑定值，不能从部署时 current 常量回填。首发前直接使用行载荷版本 `1`，不保留旧载荷或注册表，也不新增数据库列或 migration。任何影响合法动作、结算、位置或策略节点解释的规则变化都必须发布新的规则集值。
