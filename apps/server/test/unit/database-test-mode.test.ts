...
describe('database integration test mode', () => {
  ...
  test('accepts the M4.5 milestone selected by either remote suite', () => {
    expect(
      loadDatabaseTestMode({
        DATABASE_TEST_ENTRYPOINT: 'run-database-integration-tests',
        DATABASE_TEST_SCOPE: 'milestone',
        DATABASE_TEST_MILESTONE: 'm45',
        DATABASE_TEST_RUN_ID: '0123456789abcdef',
      }),
    ).toEqual({
      enabled: true,
  ...
  test('accepts the M4.7 milestone selected by either remote suite', () => {
    expect(
      loadDatabaseTestMode({
        DATABASE_TEST_ENTRYPOINT: 'run-database-integration-tests',
        DATABASE_TEST_SCOPE: 'milestone',
        DATABASE_TEST_MILESTONE: 'm47',
        DATABASE_TEST_RUN_ID: '0123456789abcdef',
      }),
    ).toEqual({
      enabled: true,
  ...
  test('accepts the M4.8 milestone selected by either remote suite', () => {
    expect(
      loadDatabaseTestMode({
        DATABASE_TEST_ENTRYPOINT: 'run-database-integration-tests',
        DATABASE_TEST_SCOPE: 'milestone',
        DATABASE_TEST_MILESTONE: 'm48',
        DATABASE_TEST_RUN_ID: '0123456789abcdef',
      }),
    ).toEqual({
      enabled: true,
