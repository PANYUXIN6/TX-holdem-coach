...
更新时间：2026-09-07（M0–M2、M3.1–M3.7、M4.1–M4.10、M5.1–M5.4 已实现；M3.8 主体接线已由 M4.10 落地，待按专项设计收口；M5.1 已通过 m51 database milestone，M5.2 已通过 m52 PostgreSQL E2E，M5.3 与 M5.4 均已通过各自 database 与 PostgreSQL E2E milestones）
...
- `apps/server/src/http/`：M3.5/M3.7/M5.2–M5.4 HTTP 适配边界；`create-app.ts` 组合安全中间件与路由，`GET|HEAD /api/hands/:handId`、`GET|HEAD /api/hands` 与 `GET|HEAD /api/statistics` 可携带 query；统计路由在 8192 字节预算内先严格验证百分号/UTF-8，再规范化单值查询，其余接口仍拒绝 query；HTTP 不读取数据库或拼装公开快照。
...
- `packages/contracts/src/index.ts`：公开协议唯一 Schema 边界；除当前手时间线和最近完成手摘要外，M5.3 定义严格的历史列表筛选、卡片、历史人物快照和分页响应；M5.4 定义严格的统计查询、人物快照条件、分子/分母/百分比一致的固定汇总响应，不依赖服务器私有类型。
