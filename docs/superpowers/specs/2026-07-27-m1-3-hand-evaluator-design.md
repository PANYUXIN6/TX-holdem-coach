...
`comparisonGrade` 的牌面点数使用项目既有顺序 `2 = 0` 至 `A = 12`。首位是固定的标准牌型级别 `highCard = 0` 至 `straightFlush = 8`；其余五位来自 `pokersolver` 已按比较顺序排定的最佳五张。等级按字典序由大到小比较，且只用于稳定存储、显示和诊断。
...
`pokersolver` 的最佳成牌必须恰好为五张；适配器把它们重新投影为纯 `Card`，不返回资源 `code`、英文描述、原始牌组对象或任何第三方类型。

中文名称固定为：高牌、一对、两对、三条、顺子、同花、葫芦、四条、同花顺。`Royal Flush` 仍属于 `straightFlush`，但 `displayName` 显示为“皇家同花顺”。未知的第三方牌型名称或未返回五张最佳牌属于适配器故障，必须抛出明确错误，不能产出部分结果。
...
4. `A-2-3-4-5` 轮子顺子低于六高顺子。
5. 五张公共牌独立成牌时，双方相同的完全平局。
6. 5、6、7 张输入都能求解，且 6/7 张会选出最佳五张。
