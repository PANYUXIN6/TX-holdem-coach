...
更新时间：2026-07-26（M0.2 已实现固定用户座位、Provider 共享协议与八个预设人物规格；M0.3 已实现 Provider 初始配置投影；M1.1 已实现私有扑克状态构造与共用行动命令）
