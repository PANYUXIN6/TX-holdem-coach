...
更新时间：2026-08-24（M0–M2、M3.1–M3.7、M4.1–M4.4、首发前预建面收敛及远程测试分层已同步）
...
- `docs/superpowers/specs/2026-08-23-m4-4-authoritative-player-observation-information-boundary-design.md`：M4.4 正式事实源；已实现短事务内 `Session → AgentRun` 共享锁、Owner/决策身份/lease/fencing/deadline 原子复验、字段级白名单 Player 观察、认证深冻结 `PlayerVisibleState` 与第一道信息防火墙。第二、第三道 Guard 仍由 M4.6 交付，三道总验收尚未闭环。
...
- `apps/server/test/`：`unit/` 与 `service/` 是离线层；前者覆盖领域、Codec、Repository/Coordinator 窄端口、M4.3 Context/Capability/Gateway 及 M4.4 观察/Guard/依赖边界，后者覆盖 HTTP、Provider 与应用适配；`vitest.coverage.config.ts` 也只收集这两层。`integration/database-infrastructure.test.ts` 注册 M2.2–M2.8、M3.5 设置持久化及 M4.4 观察 Repository/共享锁事实；`database-m44-assertions.ts` 不贯穿 Runtime 编排。`integration/postgres-application-e2e.test.ts` 独立注册 M3.1–M3.7、M4.2–M4.4 的跨应用层 PostgreSQL 验收；`postgres-e2e-m44-assertions.ts` 从真实 Session 与 leased/running Player Run 贯穿到认证观察。两套入口只有显式启动器标记存在时才运行，默认 `verify` 均不收集；日常 `db:test:integration` 仍只执行 `prefix → migrate → exact`。
...
- `apps/server/src/poker/betting-projection.ts`：共享纯下注投影内核；从最小公开座位/下注轮事实生成合法动作、筹码转移、full raise/reopening、响应者和街道关闭结果，由权威扑克引擎与 M4.4 Builder/第一 Guard 共同消费，不依赖 Session、Agent、Persistence 或 Provider。
...
- `apps/server/src/agents/player/` 与 `apps/server/src/agents/coach/`：各自拥有唯一当前 Definition、预算政策、Manifest grants 和独立认证 Route Policy；Player 额外拥有恢复后的泛型 `PlayerObservationAuthorityPort<TProjection>` 及其 `owner + identity` 输入实例化，只接收认证 `PlayerVisibleState`。当前仍没有生产业务 Context/Prompt、第二/第三 Guard、Commit Gate 或 Runtime executor，两个 Runtime 不共享业务 Context、结果或提交端口。
...
本节描述 M4.4 完成后的代码现状；会话版本仍属于 `PrivateTableState`，纯扑克规则、审计 Repository 与 Agent Foundation 协议都不自行推进它。

根 pnpm 脚本编排三个 workspace；`verify` 固定执行格式检查、类型检查与后端分类测试，且不读取模型 Key、数据库凭据或联网。Server 启动门仍止于 M3.7 的 HTTP/投影/SSE 组合。M4.4 已提供“单 Run authority 端口 → 权威短事务读取 → 白名单构建 → 第一 Guard → 认证观察”，但 M4.5/M4.6 预处理、决策包和后两道 Guard、M4.7 Commit Gate 与 `bootstrap.ts` 接线仍不存在，因此默认启动和 `verify` 均不会发起模型请求。
