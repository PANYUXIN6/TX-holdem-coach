...
本文件只保留全仓库硬约束；专题规则按链接渐进加载。

## 实现原则

- 遵循现有模块边界。
- 不得无依据新增 legacy/fallback；删除既有代码、依赖、测试、文档或兼容路径前，必须确认消费者、外部契约和数据影响，并取得明确授权。
- 保留用户已有修改。
- 并发边界从严处理。
- 现有测试和断言在需求被证明确已改变前视为契约。
...
- 开发以及测试过程中如果需要连接远程数据库，需要中断询问我网络是否可用，然后再继续执行。
