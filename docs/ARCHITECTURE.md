...
更新时间：2026-09-05（M0–M2、M3.1–M3.7、M4.1–M4.10、M5.1–M5.3 已实现；M3.8 主体接线已由 M4.10 落地，待按专项设计收口；M5.3 已通过 m53 database 与 PostgreSQL E2E milestones）
...
- M3.5 已新增 `src/http/`、`src/providers/`、`src/settings/` 与删除应用服务。`createApp()` 是唯一 HTTP 组合入口，先执行回环 Host、精确 Origin、JSON MIME、流式 64 KiB、严格 query 与安全响应头门禁，再由路由严格解析 Contracts、调用应用端口并复验公开输出；M5.2 的 `GET|HEAD /api/hands/:handId` 仅接受一个 `view`，M5.3 的 `GET|HEAD /api/hands` 在独立 8192 字节 query 预算内接受严格的筛选和游标，其余接口仍拒绝 query。请求日志只记录 requestId、方法、路由模板、状态、稳定错误码和耗时。Provider 检测只访问固定 models 端点，使用覆盖响应正文读取的端到端 10 秒超时、流式 256 KiB 响应上限、同 Provider 单飞和进程缓存，并只记录 Provider、公开分类和耗时；Player 设置部分更新通过默认行竞争与 `FOR UPDATE` 在锁内重读、合并、完整复验和更新。M3.5 初始里程碑安装健康、Provider、设置、人物和删除路由；M3.6 已补齐场次创建、读取与命令生产适配器，M3.7 已补齐 SSE 路由，M5.2/M5.3 已补齐完成手详情与列表路由。详细事实源见 [M3.5 设计](./superpowers/specs/2026-08-11-m3-5-hono-api-error-mapping-design.md)。
...
- `packages/contracts` 提供前后端共享的严格 Zod 外部协议：命令、公开快照、结构化合法动作、人物公开摘要与创建选择、M3.2 创建请求/成功快照响应、DeepSeek 健康/设置、M3.5 健康/Player 设置/人物/删除协议、M5.2 详情与 M5.3 列表输入/响应、HTTP/SSE 信封和错误响应。Contracts 不包含数据库行模型、人物 Prompt／完整模型配置、牌堆、burn card、未公开底牌、私有下注轮或迁移结果。创建请求只允许 `currentCatalog` 的 5–8 个唯一 AI 选择或无额外字段的 `latestEnded`。
...
[非 Agent 运行时架构重基线](./superpowers/specs/2026-07-28-non-agent-runtime-architecture-rebaseline.md)已确认；M1.R/M1.8/M1.9、M0.2、M2.1–M2.8、M3.1–M3.7 与 M5.1–M5.3 已实施，M3.8 按 M4 依赖后置，M5.4–M7 仍待完成：
